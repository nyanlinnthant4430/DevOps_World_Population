...
/**
 * Generates a report showing population by continent.
 */
public class ReportPopulationByContinent {

    private static final Logger LOGGER =
            Logger.getLogger(ReportPopulationByContinent.class.getName());
    ...
    public static void generateReport(Connection con) {
        try {
            ...
            ResultSet rset = stmt.executeQuery(
                    "SELECT t.Continent AS name, t.total_population, " +
                            ...
                            "FROM (SELECT Continent, SUM(Population) AS total_population FROM country GROUP BY Continent) t " +
                            "LEFT JOIN (SELECT country.Continent, SUM(city.Population) AS city_population " +
                            "FROM city JOIN country ON city.CountryCode = country.Code GROUP BY country.Continent) cities " +
                            "ON t.Continent = cities.Continent ORDER BY t.total_population DESC;"
            );

            LinkedList<ContinentPopulation> rows = new LinkedList<>();

            while (rset.next()) {
                rows.add(new ContinentPopulation(
                        rset.getString("name"),
            ...
            table.addRule();
            table.addRow("Continent", "Total Population", "In Cities (%, count)", "Not in Cities (%, count)");
            table.addRule();
            ...
            LOGGER.log(Level.SEVERE,
                    "Error generating Population by Continent report", e);
        }
    }
}
